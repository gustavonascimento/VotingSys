...
class Voter {
   ...
   */
  constructor(voterId, registrarId, firstName, lastName, password) {
    ...
    if (this.validateVoter(voterId) && this.validateRegistrar(registrarId)) {
      ...
      throw new Error('O CPF do Eleitor não é válido.');
    } else {
      throw new Error('O CPF do Eleitor não é válido.');
    }
  ...
  async validateVoter(voterId) {
    //VoterId error checking here, i.e. check if valid CPF, or state ID
    if (voterId) {
      return true;
    } else {
      return false;
