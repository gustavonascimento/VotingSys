...
exports.connectToNetwork = async function (userName) {
  ...
  try {
    ...
    if (!userExists) {
      ...
      let response = {};
      response.error = 'O CPF do usuário ' + userName + ' não existe na carteira do Blockchain. Registre ' + userName + ' primeiro';
      return response;
    }
...
exports.registerVoter = async function (voterId, registrarId, firstName, lastName) {
  ...
  if (!registrarId || !voterId || !firstName || !lastName) {
    let response = {};
    response.error = 'Error! Você precisa preencher todos os campos para se registrar!';
    return response;
  }
  ...
  try {
    ...
    if (userExists) {
      ...
      response.error = `Error! Já existe um usuário com o CPF ${voterId} registrado. Favor inserir um CPF diferente.`;
      return response;
    }
    ...
    if (!adminExists) {
      ...
      let response = {};
      response.error = `Não existe o usuário ${appAdmin} na carteira do Blockchain. 
        Rode o script enrollAdmin.js para criar um usuário administrador`;
      return response;
    }
    ...
    console.log(`Successfully registered voter ${firstName} ${lastName}. Utilize o CPF ${voterId} para realizar o login acima.`);
    let response = `Eleitor registrado com sucesso ${firstName} ${lastName}. Utilize o CPF ${voterId} para realizar o login acima.`;
    return response;
  } catch (error) {
