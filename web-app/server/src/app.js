...
app.get('/getCurrentStanding', async (req, res) => {
  ...
  let networkObj = await network.connectToNetwork(appAdmin);
  let response = await network.invoke(networkObj, true, 'queryByObjectTypeGraph', 'votableItem');
  let parsedResponse = await JSON.parse(response);
...
app.get('/getCurrentStandingAdmin', async (req, res) => {
...
app.post('/registerVoter', async (req, res) => {
  ...
  let response = await network.registerVoter(voterId, req.body.registrarId, req.body.firstName, req.body.lastName, req.body.password);
  ...
  if (response.error) {
    ...
    if (invokeResponse.error) {
      ...
      parsedResponse += '. Volte a página inicial e utilize o seu CPF para realizar o login.';
      res.send(parsedResponse);
...
app.post('/validateVoter', async (req, res) => {
  console.log('req.body: ');
  console.log(req.body);
  let networkObj = await network.connectToNetwork(req.body.voterId, req.body.password);
  console.log('networkobj: ');
  console.log(util.inspect(networkObj));
  ...
  if (invokeResponse.error) {
  ...
  } else if(req.body.voterId == 'administrador' && req.body.password == 'zukbxxkd'){
    let response = {};
    response.error = 'admin';
    res.send(response);
  } else if(req.body.voterId == 'administrador' ) {
    let response = {};
    ...
    if (parsedResponse.ballotCast) {
      let response = {};
      response.error = 'Este eleitor já votou, é aceito apenas um voto por eleitor!';
      res.send(response);
    }
...
app.post('/validateVoterAdmin', async (req, res) => {
  console.log('req.body: ');
  console.log(req.body);
  let networkObj = await network.connectToNetwork(req.body.voterId, req.body.password);
  console.log('networkobj: ');
